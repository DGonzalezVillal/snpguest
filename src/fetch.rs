...
pub fn get_processor_model(att_report: AttestationReport) -> Result<ProcType> {
    if att_report.version < 3 {
        return Err(anyhow::anyhow!(
            "Attestation report version is too low to get cpu model"
        ));
    }
    let cpu_fam = att_report.cpuid_fam_id.unwrap();
    let cpu_mod = att_report.cpuid_mod_id.unwrap();
...
mod cert_authority {
    ...
    pub struct Args {
        ...
        #[arg(
            ...
            required_unless_present = "processor_model",
    ...
    pub fn fetch_ca(args: Args) -> Result<()> {
        let proc_model = if let Some(processor_model) = args.processor_model {
            ...
            return Err(anyhow::anyhow!("No processor model provided"));
        };
        // Get certs from kds
    ...
    pub fn request_vcek_kds(
        ...
        let vcek_url: String = match processor_model {
            ProcType::Turin => {
                if att_report.version < 3 {
                    return Err(anyhow::anyhow!(
                        "Turin attestation is not supported in version 2 reports"
                    ));
                } else {
                    format!(
                    ...
                    {hw_id}?fmcSPL={:02}&blSPL={:02}&teeSPL={:02}&snpSPL={:02}&ucodeSPL={:02}",
                        processor_model.to_kds_url(),
                        att_report.reported_tcb.fmc.unwrap(),
